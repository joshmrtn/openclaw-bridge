...
type LinkEntry = {
    ...
    heartbeat?: {
        ...
        target?: string;             // raw recipient id (channel id, or owner user id when kind=dm)
        kind?: "dm" | "channel";     // #250: dm => DM the target (user:<id>); else post (channel:<id>)
...
export function shouldFireIdleHeartbeat(
    state: { lastMessageAt: number; idleHeartbeatFiredAt: number },
    idleMs: number,
    now: number,
): boolean {
...
export function isTransientGenerateStatus(status: number): boolean {
...
export function parseBackoffSchedule(raw: string | undefined): number[] {
    if (!raw) return [...DEFAULT_BACKOFF_SCHEDULE];
    const parsed = raw
        .split(",")
        .map((s) => Number(s.trim()))
...
export async function generateWithRetry(
    ...
    opts: { schedule: number[]; sleep: (ms: number) => Promise<void> }
): Promise<{ status: number; body: any }> {
...
export function buildUnavailableText(
    character: string,
    status: number,
    linkEntry: { fallback_message?: string }
): string {
    if (linkEntry.fallback_message) return linkEntry.fallback_message;
 ...
 */
export function buildOutboundTarget(kind: string | undefined, id: string | undefined): string {
    const rawId = String(id ?? "").trim();
    if (!rawId) return "";
    if (/^(user|channel|group|conversation|room|dm):/i.test(rawId)) return rawId;
 ...
 */
export function resolveSendTarget(action: any, ctx: any): { channelId: string; to: string } {
...
export async function readWorkspaceFile(
    ...
    relPath: string,
    maxChars = 4000,
): Promise<{ outcome: string; content: string | null }> {
    ...
    try {
        ...
        if (err?.code === "ENOENT") return { outcome: "not found", content: null };
        throw err;
    }
}
    ...
    register(api) {
        ...
        api.on("before_dispatch", async (event: any, ctx: any) => {
            ...
            if (delivery.kind === "reply") {
                console.log(
                    `[openclaw-bridge] ST responded (${delivery.text.length} chars) — delivering synthetic reply`
                );
            ...
            if (delivery.kind === "silent") {
                ...
                console.warn(
                    `[openclaw-bridge] ST returned an empty 200 response for ${character} — claiming and staying silent (possible ST/link misconfig)`
                );
                return { handled: true, text: "" };
            }
            ...
            console.warn(
                `[openclaw-bridge] ST generation failed for ${character} (status ${result.status}) after retries — delivering unavailable reply, NOT invoking agent`
            );
            try {
                await postJson(`${ST_BASE}/api/plugins/openclaw-bridge/log-action`, token, {
                    character,
