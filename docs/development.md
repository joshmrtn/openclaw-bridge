...
The full tier additionally passes `--volumes` to flush the `character-links` named volume; `character-links-init` and `link-setup` recreate it correctly on every run.
