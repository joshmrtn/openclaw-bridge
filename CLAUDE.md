...
**Running the suite:** Just run `npm run test:all` with `run_in_background: true` — the script automatically writes output to `/tmp/test-all.log`. Do NOT pipe through `tee` or redirect to any other file. The suite takes 15–30 minutes — after the background task completes, read `/tmp/test-all.log` directly (e.g. `tail -100 /tmp/test-all.log`). Do NOT poll repeatedly — check at most once every 5 minutes, or wait for the background task completion notification.
...
**Full-tier E2E invariant — the rule that keeps getting forgotten.** The full tier (`docker/full/`) exists to prove the WHOLE system across its lifecycle: install (`setup.sh`) → use → uninstall (`uninstall.sh`), with real OC + real ST + real openclaw-bridge. The only fakes are the LLM (one `fake-openai` OpenAI-compatible mock serving both ST and OC's agent) and the "discord" channel (qa-bus). Behavioural tests MUST drive the real path: inject a message into qa-bus and assert on qa-bus output — not call ST's `/generate` directly. **Calling `/generate` directly bypasses OC and silently demotes a full-tier test to a slow dockerized unit test** (which the unit tier already covers). Litmus test: "Could this be driven by injecting into qa-bus and asserting on qa-bus output?" If yes, it MUST go that way. A direct `/generate` call is only acceptable for behaviour OC genuinely cannot drive — HTTP auth/CSRF header crafting, plugin/WS-layer mechanics OC never touches, induced-infra-failure resilience, or same-instant concurrency — and each such call MUST carry a `// FULL-PATH-EXCEPTION: <reason>` comment. This is enforced by the unit-tier guardrail `st-plugin/tests/full-tier-invariant.test.js`, which fails CI if any direct `/generate` call is unjustified. To clear a violation, prefer rewriting through qa-bus; annotate only when the full path truly cannot drive it.
