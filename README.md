...
handles the channel layer (Discord, Telegram, autonomous scheduling) but never
owns the character.

The practical difference comes down to context injection. OC's native memory
system is capable but relatively flat. ST's pipeline — multiple lorebooks,
conditional keyword activation, fine-grained token budgets, author's notes —
produces more consistent and nuanced character voices. If that fidelity matters
to you, bridging is the better path. If you just need the character reachable
on Discord and are happy letting OC own the context, migration is simpler.
